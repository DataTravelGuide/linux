...
/**
 *  fm10k_get_headlen - determine size of header for LRO/GRO
 ...
 **/
static unsigned int fm10k_get_headlen(unsigned char *data,
			    ...
			    struct sk_buff *skb)
{
	 ...
	 * RH Note This should be:
	 * pull_len = eth_get_headlen(va, FM10K_RX_HDR_LEN);
	 * but we don't have the infrastrucutre to support eth_get_headlen yet
	 * so we dummy it up with our own internal version (taken from
	 * igb_get_headlen)
	 */
	pull_len = fm10k_get_headlen(va, FM10K_RX_HDR_LEN);
