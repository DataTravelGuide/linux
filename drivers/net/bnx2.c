...
bnx2_set_flags(struct net_device *dev, u32 data)
{
	...
	/* ethtool_op_set_flags handles only RXHASH and LRO */
	rc = ethtool_op_set_flags(dev, data & ETH_FLAG_RXHASH);
	if (rc)
		return rc;

	/* ETH_FLAG_RXVLAN & ETH_FLAG_TXVLAN flags has to be handled here */
	if (data & ETH_FLAG_RXVLAN)
		dev->features |= ETH_FLAG_RXVLAN;
	else
		dev->features &= ~ETH_FLAG_RXVLAN;
	if (data & ETH_FLAG_TXVLAN)
		dev->features |= ETH_FLAG_TXVLAN;
	else
		dev->features &= ~ETH_FLAG_TXVLAN;
