	...
	*csrows = 0;
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		...
		if (type == SANDY_BRIDGE) {
			/* Device 15 functions 2 - 5  */
			pdev = get_pdev_same_bus(bus, PCI_DEVICE_ID_INTEL_SBRIDGE_IMC_TAD0 + i);
		} else {
			/* In 2HA mode, channels 2, 3 are in a different
			 * device */
			if (i < 2 || !ha2)
				pdev = get_pdev_same_bus(bus, PCI_DEVICE_ID_INTEL_IBRIDGE_IMC_HA0_TAD0 + i);
			else
				pdev = get_pdev_same_bus(bus, PCI_DEVICE_ID_INTEL_IBRIDGE_IMC_HA1_TAD0 + (i - 2));
		}
		if (!pdev) {
			sbridge_printk(KERN_ERR, "Couldn't find PCI device "
						 "%2x.%02d.%d!!!\n",
