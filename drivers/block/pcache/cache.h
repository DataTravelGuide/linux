...
/*
 * pcache_cache_ctrl points to the address of the first pcache_cache_seg_ctrl.
 * It extends the control structure of the first cache segment, storing
 * information relevant to the entire pcache_cache.
 */
#define PCACHE_CACHE_CTRL_OFF PCACHE_SEG_INFO_SIZE
#define PCACHE_CACHE_CTRL_SIZE PAGE_SIZE

struct pcache_cache_used_segs {
	struct pcache_meta_header header;
	u32	used_segs;
};

/* pcache_cache_info is a part of pcache_backend_info and can only be updated
 * by the backend. Its integrity is ensured by a unified meta_header.
 * In contrast, the information within pcache_cache_ctrl may be updated by blkdev.
 * Each piece of data in pcache_cache_ctrl has its own meta_header to ensure
...
struct pcache_cache_ctrl {
	...
	/* Updated by blkdev */
	struct pcache_cache_used_segs used_segs[PCACHE_META_INDEX_MAX];
};
...
struct pcache_cache_key_onmedia {
	...
	u32 cache_seg_off;
	u64 seg_gen;
#ifdef CONFIG_PCACHE_CACHE_DATA_CRC
	u32 data_crc;
#endif
};
