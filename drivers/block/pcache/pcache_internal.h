...
#define PCACHE_KB                  (1024)                      /* 1 Kilobyte in bytes */
#define PCACHE_MB                  (1024 * PCACHE_KB)           /* 1 Megabyte in bytes */

#define PCACHE_CACHE_DEV_MAX       1024                        /* Maximum number of cache_dev instances */
#define PCACHE_PATH_LEN            128
#define PCACHE_NAME_LEN            32

#define PCACHE_QUEUES_MAX          128                         /* Maximum number of I/O queues */
#define PCACHE_HANDLERS_MAX        128                         /* Maximum number of handlers */

#define PCACHE_PART_SHIFT          4                           /* Bit shift for partition identifier */
#define PCACHE_DRV_NAME         "pcache"                    /* Default driver name for pcache */
#define PCACHE_DEV_NAME_LEN        32                          /* Maximum device name length */

#define PCACHE_HB_INTERVAL         msecs_to_jiffies(5000)      /* Heartbeat interval in jiffies (5 seconds) */
#define PCACHE_HB_TIMEOUT          (30 * 1000)                 /* Heartbeat timeout in milliseconds (30 seconds) */
...
#define PCACHE_META_CRC_SEED		0x1234
...
{
	return crc32(PCACHE_META_CRC_SEED, (void *)header + 4, meta_size - 4);  /* CRC calculated starting after the crc field */
}
					 ...
					 u32 meta_size)
{
	...
	for (i = 0; i < PCACHE_META_INDEX_MAX; i++) {
		...
		//pr_err("meta: %p crc: %u, meta_crc: %u", meta, meta->crc, pcache_meta_crc(meta, meta_size));
