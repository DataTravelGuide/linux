...
static int cache_dev_dax_init(struct pcache_cache_dev *cache_dev, char *path)
{
	...
	if (mapped_pages == total_pages) {
		...
		if (!pages) {
			cache_dev_err(cache_dev, "failed to alloc array for pages, total_pages: %lu.\n", total_pages);
			ret = -ENOMEM;
			goto unlock;
		}
...
static int cache_dev_format(struct pcache_cache_dev *cache_dev, bool force)
{
	...
	flags |= PCACHE_SB_F_CACHE_DATA_CRC;
#endif
...
#ifndef CONFIG_PCACHE_CACHE_DATA_CRC
	if (flags & PCACHE_SB_F_CACHE_DATA_CRC) {
		cache_dev_err(cache_dev, "cache_dev expects PCACHE_CACHE_DATA_CRC enabled.\n");
		return -EOPNOTSUPP;
	}
#endif
	return 0;
