						 ...
						 int *errp)
{
	...
	if (xfrm_id_proto_match(p->proto, IPSEC_PROTO_ANY)) {
		err = -ESRCH;
		x = xfrm_state_lookup_with_mark(net, DUMMY_MARK, &p->daddr, p->spi, p->proto, p->family);
	} else {
		...
		err = -ESRCH;
		x = xfrm_state_lookup_with_mark_byaddr(net, DUMMY_MARK, &p->daddr, saddr,
					     p->proto, p->family);
	}
		...
		struct nlattr **attrs)
{
	...
	x = xfrm_state_lookup_with_mark(net, DUMMY_MARK, &id->daddr, id->spi, id->proto, id->family);
	if (x == NULL) {
		kfree_skb(r_skb);
		return -ESRCH;
	}
		...
		struct nlattr **attrs)
{
	...
	x = xfrm_state_lookup_with_mark(net, DUMMY_MARK, &p->sa_id.daddr, p->sa_id.spi, p->sa_id.proto, p->sa_id.family);
	if (x == NULL)
		return -ESRCH;
		...
		struct nlattr **attrs)
{
	...
	x = xfrm_state_lookup_with_mark(net, DUMMY_MARK, &p->id.daddr, p->id.spi, p->id.proto, p->family);
