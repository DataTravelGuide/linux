...
static void cpumf_measurement_alert(__u16 ext_code)
{
	struct cpu_hw_events *cpuhw;

	if (!(S390_lowcore.ext_params & CPU_MF_INT_CF_MASK))
		return;
	...
	if (S390_lowcore.ext_params & CPU_MF_INT_CF_CACA)
		qctri(&cpuhw->info);

	/* loss of counter data alert */
	if (S390_lowcore.ext_params & CPU_MF_INT_CF_LCDA)
