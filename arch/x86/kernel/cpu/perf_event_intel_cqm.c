...
/**
 * Having local copy of cgroup_is_descendant added upstream at
 *   78574cf981cd cgroup: implement cgroup_is_descendant()
 * RHEL6 still exports old version checking the cgroup and task.
 *
 * __cgroup_is_descendant - test ancestry
 ...
 */
static bool __cgroup_is_descendant(struct cgroup *cgrp, struct cgroup *ancestor)
{
	while (cgrp) {
		if (cgrp == ancestor)
			return true;
...
static bool __conflict_event(struct perf_event *a, struct perf_event *b)
{
	...
	if (a->cgrp && b->cgrp) {
		...
		if (__cgroup_is_descendant(ac->css.cgroup, bc->css.cgroup) ||
		    __cgroup_is_descendant(bc->css.cgroup, ac->css.cgroup))
			return true;
...
{
	struct intel_cqm_state *state = this_cpu_ptr(&per_cpu_var(cqm_state));
...
static int __intel_cqm_cpu_notifier(struct notifier_block *nb,
...
static struct notifier_block __cpuinitdata intel_cqm_cpu_notifier = {
	.notifier_call = __intel_cqm_cpu_notifier ,
};
...
static int __init intel_cqm_init(void)
{
	...
	cqm_l3_scale = boot_cpu_data_rh.x86_cache_occ_scale;
	...
	register_cpu_notifier(&intel_cqm_cpu_notifier);
