...
static inline void __remove_ddw(struct device_node *np, const u32 *ddr_avail, u64 liobn)
{
	int ret;
...
static void remove_ddw(struct device_node *np)
{
	...
	__remove_ddw(np, ddr_avail, liobn);
}
				...
				u32 ddw_restore_token, unsigned long liobn)
{
	...
	dev_info(&dev->dev,
		"ibm,reset-pe-dma-windows(%x) %x %x %x returned %d\n",
		 ddw_restore_token, cfg_addr, (u32)BUID_HI(buid),
		 (u32)BUID_LO(buid), ret);
}
...
static u64 enable_ddw(struct pci_dev *dev, struct device_node *pdn)
{
	...
	if (ddw_restore_token) {
		...
		__remove_ddw(pdn, ddr_avail, liobn);
	}
