...
static unsigned long get_fadump_area_size(void)
{
	...
	size += sizeof(struct elf_phdr) * (lmb_num_regions(memory) + 2);
...
int __init fadump_reserve_mem(void)
{
	 ...
	 */
	if (memory_limit && memory_limit < lmb_end_of_DRAM()) {
		size = get_fadump_area_size();
		if ((memory_limit + size) < lmb_end_of_DRAM())
			memory_limit += size;
		else
	...
	if (memory_limit)
		...
		memory_boundary = lmb_end_of_DRAM();
...
/*
 * Traverse through lmb structure and setup crash memory ranges. These
 * ranges will be used create PT_LOAD program headers in elfcore header.
 */
static void fadump_setup_crash_memory_ranges(void)
{
	struct lmb_property *reg;
