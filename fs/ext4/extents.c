			      ...
			      int flags)
{
	...
	if (map->m_lblk + map->m_len < ee_block + ee_len) {
		...
		flags1 = flags | EXT4_GET_BLOCKS_DIO;
		if (uninitialized)
			...
			int flags)
{
	...
	if (cache_type && ((flags & EXT4_GET_BLOCKS_PUNCH_OUT_EXT) == 0)) {
	...
	if (ex) {
		...
		if (in_range(iblock, ee_block, ee_len)) {
			 ...
			 * end of the extent
			 */
			punched_out = allocated < max_blocks ?
				allocated : max_blocks;
			...
			err = ext4_ext_remove_space(inode, iblock,
				iblock + punched_out);
...
void ext4_ext_truncate(struct inode *inode)
{
	...
	err = ext4_ext_remove_space(inode, last_block, EXT_MAX_BLOCKS);
