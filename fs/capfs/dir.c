...
static struct dentry *lookup_backing_meta_file(struct capfs_info *tfs, struct dentry *backing_dir_dentry)
{
	struct mnt_idmap *mnt_idmap;
	...
	meta_dentry = lookup_one_unlocked(mnt_idmap, "meta", backing_dir_dentry, strlen("meta"));
	if (IS_ERR(meta_dentry)) {
		capfs_err("lookup_one_unlocked for 'meta' failed: %ld\n", PTR_ERR(meta_dentry));
		result = meta_dentry;
		goto err;
	}

	if (d_really_is_negative(meta_dentry)) {
		capfs_err("backing meta file not found in backing subdir\n");
		result = ERR_PTR(-ENOENT);
		goto put_dentry;
	}

	pr_err("print meta_dentry");
	capfs_print_dentry(meta_dentry);
...
static struct dentry *capfs_lookup(struct inode *dir, struct dentry *dentry, unsigned int flags)
{
	...
	meta_dentry = lookup_backing_meta_file(tfs, backing_subdir_dentry);
	if (IS_ERR_OR_NULL(meta_dentry)) {
		result = ERR_PTR(-EIO);
		goto put_data_dentry;
	}

	ti_param.backing_dentry = backing_subdir_dentry;
	ti_param.backing_data_file_dentry = data_dentry;
	ti_param.backing_meta_file_dentry = meta_dentry;
			...
			bool excl)
{
	...
	/* Step 0: Override credentials with creator's credentials */
	tfs = capfs_info_i(dir);
	old_cred = override_creds(tfs->creator_cred);

	/* Step 1: Get the backing mount idmap */
	...
	if (IS_ERR(backing_mnt_idmap)) {
		...
		goto put_backing_mnt_idmap;
	}

	/* Step 2: Get the backing directory dentry */
	...
	if (!backing_dir) {
		...
		ret = -ENOENT;
		goto put_backing_dir_dentry;
	}

	/* Step 3: Lookup backing subdirectory dentry */
	backing_subdir_dentry = lookup_one_unlocked(backing_mnt_idmap, dentry->d_name.name,
							backing_dir_dentry, dentry->d_name.len);
	if (IS_ERR(backing_subdir_dentry)) {
	...
	if (d_really_is_positive(backing_subdir_dentry)) {
		...
		ret = -EEXIST;
		goto put_backing_subdir_dentry;
	}

	/* Step 4: Create subdirectory in the backing filesystem */
	...
	data_dentry = lookup_one_unlocked(capfs_backing_mnt_idmap(dir), "data", backing_subdir_dentry, strlen("data"));
	if (IS_ERR(data_dentry)) {
		ret = PTR_ERR(data_dentry);
		printk(KERN_ERR "capfs: lookup_one for 'data' failed with err=%d\n", ret);
		goto rmdir_backing_subdir;
	}
	...
	/* Step 6: Create the 'data' file in the backing filesystem */
	...
	/* Step 7: Create 'meta' file and write metadata */
	unsigned int file_size = d_inode(data_dentry)->i_size;
	unsigned int seg_num = (file_size + (1024 * 1024 - 1)) / (1024 * 1024);  // Calculate seg_num, each segment is 1MB

	/* Allocate memory for file metadata */
	struct capfs_file_meta *file_meta = kzalloc(sizeof(*file_meta) + sizeof(struct capfs_seg_meta) * seg_num, GFP_KERNEL);
	if (!file_meta) {
		ret = -ENOMEM;
		printk(KERN_ERR "capfs: Memory allocation for file meta failed\n");
		goto put_data_dentry;
	}

	file_meta->seg_num = seg_num;

	/* Create 'meta' file */
	meta_dentry = lookup_one_unlocked(capfs_backing_mnt_idmap(dir), "meta", backing_subdir_dentry, strlen("meta"));
	if (IS_ERR(meta_dentry)) {
		ret = PTR_ERR(meta_dentry);
		printk(KERN_ERR "capfs: lookup_one for 'meta' failed with err=%d\n", ret);
		kfree(file_meta);
		goto put_data_dentry;
	}

	/* Create the 'meta' file in the backing filesystem */
	ret = vfs_create(capfs_backing_mnt_idmap(dir), d_inode(backing_subdir_dentry), meta_dentry, mode, true);
	if (ret) {
		printk(KERN_ERR "capfs: vfs_create for 'meta' in '%s' failed with err=%d\n", dentry->d_name.name, ret);
		dput(meta_dentry);
		kfree(file_meta);
		goto put_data_dentry;
	}

	/* Prepare the struct path for dentry_open */
	struct path meta_path;
	meta_path.mnt = tfs->backing_path.mnt;
	meta_path.dentry = meta_dentry;

	/* Write the metadata to 'meta' file */
	struct file *meta_file = dentry_open(&meta_path, O_WRONLY | O_CREAT, current_cred());
	if (IS_ERR(meta_file)) {
		ret = PTR_ERR(meta_file);
		printk(KERN_ERR "capfs: failed to open 'meta' file for writing with err=%d\n", ret);
		dput(meta_dentry);
		kfree(file_meta);
		goto put_data_dentry;
	}

	ret = kernel_write(meta_file, (const char *)file_meta, sizeof(*file_meta) + sizeof(struct capfs_seg_meta) * seg_num, &meta_file->f_pos);
	if (ret < 0) {
		printk(KERN_ERR "capfs: failed to write to 'meta' file with err=%d\n", ret);
		fput(meta_file);
		dput(meta_dentry);
		kfree(file_meta);
		goto put_data_dentry;
	...
	/* Step 8: Set xattr for marking the directory as fully created */
	...
	/* Step 9: Create a new inode for the capfs file */
	struct capfs_inode_param ti_param = {
		.backing_dentry = backing_subdir_dentry,
		.backing_data_file_dentry = data_dentry,
		.backing_meta_file_dentry = meta_dentry,
		.mode = S_IFREG
	};
	...
	/* Step 10: Release 'data' dentry */
	dput(data_dentry);

	/* Step 11: Associate dentry with inode */
	d_instantiate(dentry, inode);

	/* Step 12: Release backing subdirectory dentry */
	dput(backing_subdir_dentry);

	pr_err("capfs_create finished.");
	revert_creds(old_cred);  // Restore original credentials
	return 0;

put_data_dentry:
	dput(data_dentry);

rmdir_backing_subdir:
	vfs_rmdir(capfs_backing_mnt_idmap(dir), backing_dir, backing_subdir_dentry);
	dput(backing_subdir_dentry);

put_backing_subdir_dentry:
	dput(backing_subdir_dentry);

put_backing_dir_dentry:
	dput(backing_dir_dentry);

put_backing_mnt_idmap:
	revert_creds(old_cred);  // Restore original credentials
	return ret;
}
