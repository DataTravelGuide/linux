...
struct capfs_seg_meta {
	__le32	compressed_size;	/* times of 4K */
};

struct capfs_file_meta {
	__le64 seg_num;
	struct capfs_seg_meta segs[];
};

struct capfs_segment {
	u32 compressed_size;
};
...
static inline int capfs_backing_meta_path(struct inode *inode, struct path *path)
{
	struct capfs_info *tfs = capfs_info_i(inode);
	struct capfs_inode *ti = capfs_i(inode);

	if (!tfs->backing_path.mnt || !ti->backing_meta_file_dentry)
		return -EINVAL;

	path->mnt = tfs->backing_path.mnt;
	path->dentry = ti->backing_meta_file_dentry;
